	...
	public partial class Category {
		...
		internal static void RemoveCache(IEnumerable<CategoryInfo> items) {
			...
			foreach (var item in items) {
				keys[keysIdx++] = string.Concat("es_BLL:Category:", item.Id);
			}
		...
		public static CategoryInfo GetItem(int Id) => SqlHelper.CacheShell(string.Concat("es_BLL:Category:", Id), itemCacheTimeout, () => Select.WhereId(Id).ToOne());
		...
		async public static Task<CategoryInfo> GetItemAsync(int Id) => await SqlHelper.CacheShellAsync(string.Concat("es_BLL:Category:", Id), itemCacheTimeout, () => Select.WhereId(Id).ToOneAsync());
